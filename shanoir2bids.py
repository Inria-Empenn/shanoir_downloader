...
Search Text : "{}" \n""".format(
                    ...
                    for item in response.json()["content"]:
                        ...
                        if self.longitudinal:
                            ...
                            bids_data_dir = opj(
                                subject_dir, bids_seq_session, bids_seq_subdir
                            )
                            if self.to_automri_format:
                                bids_data_basename = "_".join(
                                    [bids_seq_session, bids_seq_name]
                                )
                            else:
                                bids_data_basename = "_".join(
                                    [subject_id, bids_seq_session, bids_seq_name]
                                )
                        else:
                        ...
                        dl_archive = glob(opj(self.dl_dir, "*" + item["id"] + "*.zip"))[
                            0
                        ]
                        ...
                        def seq_name(extension, run_num=0):
                            ...
                            """
                            if run_num > 0:
                                basename = bids_data_basename + "_run-{0}{1}".format(
                                    run_num, extension
                                )
                            else:
                        ...
                        def check_duplicated_data(
                            list_existing_file_json, file_to_add_json
                        ):
                            """
                            ...
                            for old_json in list_existing_file_json:
                                ...
                                if (
                                    ...
                                    == data_old["AcquisitionTime"]
                                ) & (data["SequenceName"] == data_old["SequenceName"]):
                        ...
                        if self.shanoir_file_type == SHANOIR_FILE_TYPE_DICOM:
                            ...
                            if not self.dcm2niix_path:
                                err_msg = "msg\n- {file}" "".format(
                                    msg=DCM2NIIX_ERR_MSG, file=self.json_config_file
                                )
                                sys.exit(err_msg)
                            if not self.dcm2niix_opts:
                                warn_msg = "msg\n- {file}" "".format(
                                    msg=DCM2NIIX_WARN_MSG, file=self.json_config_file
                                )
                                print(warn_msg)

                            dcm_files = glob(opj(tmp_dir, "*" + DCM))
                            # Define dcm2niix options
                            options = " {opts} -f {basename} -o {out}".format(
                                opts=self.dcm2niix_opts,
                                basename=bids_data_basename,
                                out=tmp_dir,
                            )
                        ...
                        if self.to_automri_format and self.add_sns:
                            ...
                            for filename in list_unzipped_files:
                                if filename.endswith(JSON):
                                    ...
                                    series_number_suffix = str(
                                        json_dataset["SeriesNumber"]
                                    )
                                    bids_data_basename = "_".join(
                                        [bids_seq_name, su_id, series_number_suffix]
                                    )
                        ...
                        for f in list_unzipped_files:  # Loop over files in the archive
                            ...
                            list_existing_f_ext = glob(
                                opj(bids_data_dir, bids_data_basename + "*" + ext)
                            )
                            ...
                            if (nf > 0) & (not duplicated_data):
                                for filename_tempo in list_unzipped_files:
                                    ...
                                    if (ext_tempo == JSON) & (ext == JSON):
                                        list_existing_f_ext_json = glob(
                                            opj(
                                                bids_data_dir,
                                                bids_data_basename + "*json",
                                            )
                                        )
                                        duplicated_data = check_duplicated_data(
                                            list_existing_f_ext_json, filename_tempo
                                        )
                            ...
                            if ext in [NIIGZ, JSON, BVAL, BVEC]:
                                if not duplicated_data:
                                    ...
                                    if nf == 0:
                                        ...
                                        fp.write(
                                            "    >> Renaming to '"
                                            + seq_name(extension=ext, run_num=2)
                                            + "'\n"
                                        )
                                    else:  # nf >= 2
                                        # At least two files exist, do not touch previous but add the right suffix to new file
                                        os.rename(
                                            f, seq_name(extension=ext, run_num=nf + 1)
                                        )
                                        fp.write(
                                            "    >> Renaming to '"
                                            + seq_name(extension=ext, run_num=nf + 1)
                                            + "'\n"
                                        )
                            else:
                                print(
                                    "[BIDS format] The extension",
                                    ext,
                        ...
                        fp.write(
                            "  >> Deleting downloaded archive " + dl_archive + "\n\n\n"
                        )
