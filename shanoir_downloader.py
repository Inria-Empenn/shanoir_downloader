...
def add_output_folder_argument(parser, required=True):
    parser.add_argument('-of', '--output_folder', required=required,
                        help='The destination folder where files will be downloaded.')
...
def add_search_arguments(parser):
    ...
    parser.add_argument('-st', '--search_text',
                        help='The search text. See the info box on https://shanoir.irisa.fr/shanoir-ng/solr-search.')


def add_configuration_arguments(parser):
    parser.add_argument('-c', '--configuration_folder', required=False,
                        help='Path to the configuration folder containing proxy.properties (Tries to use ~/.su_vX.X.X/ by default). You can also use --proxy_url to configure the proxy (in which case the proxy.properties file will be ignored).')
    parser.add_argument('-pu', '--proxy_url', required=False,
                        help='The proxy url in the format "user@host:port". The proxy password will be asked in the terminal. See --configuration_folder.')
    ...
    parser.add_argument('-lf', '--log_file', type=str,
                        help="Path to the log file. Default is output_folder/downloads.log", default=None)
    return parser


def add_ids_arguments(parser):
    parser.add_argument('-did', '--dataset_id', default='', help='The dataset id to download.')
    parser.add_argument('-dids', '--dataset_ids', default='',
                        help='Path to a file containing the dataset ids to download (a .txt file containing one dataset id per line).')
...
def init_logging(args):
    ...
    logfile = Path(args.log_file) if args.log_file else Path(
        args.output_folder) / f'downloads{datetime.datetime.now().strftime("%Y-%m-%d_%Hh%Mm%S")}.log'
...
def initialize(args, verbose=True):
    ...
    return {'domain': server_domain, 'username': username, 'verify': verify, 'proxies': proxies,
            'output_folder': output_folder, 'timeout': args.timeout}
...
def download_datasets(config, dataset_ids, file_format):
    if len(dataset_ids) > 50:
        logging.warning(
            'Cannot download more than 50 datasets at once. Please use the --search_text option instead to download the datasets one by one.')
        return
...
if __name__ == '__main__':
    ...
    elif any([getattr(args, arg_name) is not None for arg_name in
              ['dataset_id', 'dataset_ids', 'study_id', 'subject_id']]):
        download_datasets_from_ids(args)
    else:
        print(
            'You must either provide the search_text argument, or an argument in the list [dataset_id, dataset_ids, study_id, subject_id].')