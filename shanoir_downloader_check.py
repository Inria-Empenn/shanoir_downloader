...
parser.add_argument('-sc', '--skip_column', default='previously_sent', nargs='*', help='The column used to ignore data. By default, all datasets with previously_sent == 1 are ignored and not downloaded.')
...
# Drop datasets to ignore from skip_column
if args.skip_column and args.skip_column in all_datasets.columns:
	all_datasets = all_datasets[all_datasets[args.skip_column] != 1]
...
def add_downloaded_dataset(all_datasets, downloaded_datasets, missing_datasets, sequence_id, patient_name_in_dicom, series_description_in_dicom, verified):
...
while len(datasets_to_download) > 0:
	...
	for index, row in datasets_to_download.iterrows():
		...
		if shanoir_name is not None and series_description is not None:
			...
			try:
				...
				if patient_name_in_dicom != shanoir_name:
					...
					# missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'content_patient_name', f'Shanoir name {patient_name} differs in dicom: {ds.PatientName}', raw_folder)
					# continue
		...
		downloaded_datasets = add_downloaded_dataset(all_datasets, downloaded_datasets, missing_datasets, sequence_id, patient_name_in_dicom, series_description_in_dicom, verified)