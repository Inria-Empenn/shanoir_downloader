...
parser.add_argument('-ue', '--unrecoverable_errors', default=['status_code_404'], nargs='*', help='The errors which should not trigger a new download.')
...
def add_missing_dataset(missing_datasets, sequence_id, reason, message):
...
while len(datasets_to_download) > 0:
	...
	for index, row in datasets_to_download.iterrows():
		...
		destination_folder = output_folder / sequence_id / 'downloaded_archive'
		...
		try:
			...
			message += str(e)
			missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'status_code_' + str(e.response.status_code), message)
			continue
		except Exception as e:
			missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'unknown http error', str(e))
			continue
		...
		if len(zip_files) != 1:
			...
			missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'zip', message)
			continue
		...
		if len(dicom_files) == 0:
			missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'nodicom', f'No DICOM file was found in the dicom directory {dicom_folder}.')
			continue
		...
		try:
			...
			if ds.PatientName != shanoir_name:
				missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'content_shanoir_name', f'Shanoir name {shanoir_name} differs in dicom: {ds.PatientName}')
				continue

			if ds.SeriesDescription != series_description: 	# or if ds[0x0008, 0x103E].value != series_description:
				missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'content_series_description', f'Series description {series_description} differs in dicom: {ds[0x0008, 0x103E]}')
				continue
		except Exception as e:
			missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'content_read', f'Error while reading DICOM: {e}')
			continue

		# Move output_folder / sequence_id / downloaded_archive / sequence_name.zip to output_folder / sequence_id_sequence_name.zip
		dicom_zip = rename_path(dicom_zip, output_folder / f'{sequence_id}_{dicom_zip.name}')
		
		if not args.skip_anonymization:

			# Anonymize
			anonymized_dicom_folder = dicom_zip.parent / f'{dicom_zip.stem}_anonymized'
			try:
			...
			try:
				...
				missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'zip compression error', str(e))
				continue
			
			# Remove anonymized dicom
			if not args.keep_intermediate_files:
				dicom_zip.unlink()
				shutil.rmtree(anonymized_dicom_folder)

			dicom_zip = output_folder / f'{dicom_zip.stem}_anonymized.zip'
		...
		if not args.skip_encryption:
			...
			encrypted_dicom_zip = dicom_zip.parent / f'{dicom_zip.name}.gpg'
			command = ['gpg', '--output', str(encrypted_dicom_zip), '--encrypt', '--recipient', gpg_recipient, str(dicom_zip)]
			try:
				subprocess.call(command)
			except Exception as e:
				missing_datasets = add_missing_dataset(missing_datasets, sequence_id, 'encryption error', str(e))
				continue

			encrypted_dicom_zip = rename_path(encrypted_dicom_zip, output_folder / f'{sequence_id}_{encrypted_dicom_zip.name}')

			# Remove zip
			if not args.keep_intermediate_files: shutil.rmtree(dicom_zip)
