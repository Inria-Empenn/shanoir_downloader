...
if not args.skip_encryption and not args.gpg_recipient:
...
if args.dataset_ids.endswith('.csv'):
	all_datasets = pandas.read_csv(args.dataset_ids, dtype=datasets_dtype)
else:
...
while len(datasets_to_download) > 0:
	...
	for index, row in datasets_to_download.iterrows():
		...
		dicom_folder = destination_folder.parent / dicom_zip.name.replace('.zip', '')
		...
		if not args.skip_anonymization:
			...
			try:
				...
				continue
			dicom_zip = anonymized_dicom_folder.parent / f'{anonymized_dicom_folder.name}.zip'
		
		if not args.skip_encryption:

			# Encrypt the zip archive
			encrypted_dicom_zip = dicom_zip.parent / f'{dicom_zip.name}.gpg'
			command = ['gpg', '--output', str(encrypted_dicom_zip), '--encrypt', '--recipient', args.gpg_recipient, str(dicom_zip)]
			try:
